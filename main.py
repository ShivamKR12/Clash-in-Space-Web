...
async def main():
    ...
    class Game:
        def __init__(self):
            ...
            self.font = pygame.font.Font('assets/font/Oxanium-Bold.ttf', 40)
            self.explosion_frames = [pygame.image.load(f'assets/explosion/{i}.png').convert_alpha() for i in range(21)]

            # sounds
            self.laser_sound = pygame.mixer.Sound('assets/audio/laser.ogg')
            self.laser_sound.set_volume(0.5)
            self.explosion_sound = pygame.mixer.Sound('assets/audio/explosion.ogg')
            self.game_music = pygame.mixer.Sound('assets/music/game_music.ogg')
        ...
        def play_game(self, dt, events):
            for event in events:
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE and self.player.can_shoot:
                    Laser(self.laser_surf, self.player.rect.midtop, (self.all_sprites, self.laser_sprites))
                    self.player.can_shoot = False
                    self.player.laser_shoot_time = pygame.time.get_ticks()
